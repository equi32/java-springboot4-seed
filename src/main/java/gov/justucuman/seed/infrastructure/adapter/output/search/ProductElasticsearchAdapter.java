...
public class ProductElasticsearchAdapter implements ProductSearchPort {
    ...
    public void indexProduct(Product product) {
        ...
        try {
            ...
            IndexRequest<ProductDocument> request = IndexRequest.of(
                    i -> i.index(indexName).id(product.id().toString()).document(document));
    ...
    public List<Product> search(String query) {
        ...
        try {
            ...
            SearchRequest searchRequest = SearchRequest.of(s -> s.index(indexName)
                    .query(q -> q.multiMatch(m -> m.query(query).fields("name^3", "description", "category^2"))));
    ...
    public List<Product> searchByParameter(String parameter, String value) {
        ...
        try {
            ...
            SearchResponse<ProductDocument> response = openSearchClient.search(
                    s -> s.index(indexName)
                            .query(Query.of(q -> q.match(t -> t.field(parameter).query(v -> v.stringValue(value))))),
