...
public class MockServerComponent implements ArgumentAwareComponent {
    ...
    private MockServerComponent() {
        this.mockServerContainer = new MockServerContainer(DockerImageName.parse("mockserver/mockserver:5.15.0"));
    }
    ...
    public <T> void withExpectations(List<MockServerExpectation<T>> expectations) {
        try (MockServerClient client =
            ...
            expectations.forEach(expectation -> {
                log.info(
                        "Setting up MockServer expectation: {} {} -> {}",
                        expectation.getMethod(),
                        expectation.getPath(),
                        expectation.getResponseStatus());
